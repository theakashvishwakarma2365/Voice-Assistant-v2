...
class TTSService:
    def __init__(self):
        ...
        # Safe voice model path resolution
        self.voice_model_path = settings.piper_voice_model
        voice_path = Path(self.voice_model_path)
        if not voice_path.exists():
            logger.warning(f"Configured Piper voice model not found at {self.voice_model_path}")
            # Try to look for it relative to workspace root (e.g. in /voices/ folder)
            local_fallback = Path(__file__).resolve().parents[2] / "voices" / voice_path.name
            if local_fallback.exists():
                logger.info(f"Using local workspace fallback voice model: {local_fallback}")
                self.voice_model_path = str(local_fallback)
            else:
                logger.warning(
                    f"No fallback voice model found at '{local_fallback}'. "
                    "TTS requests will fail if the Piper path configuration is incorrect."
                )
